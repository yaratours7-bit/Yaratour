  ...
  try {
    ...
    if (!TWILIO_ACCOUNT_SID || !TWILIO_API_KEY || !TWILIO_API_SECRET) {
      ...
      return NextResponse.json(
        {
          error:
            'Twilio environment variables not set (required: TWILIO_ACCOUNT_SID, TWILIO_API_KEY, TWILIO_API_SECRET)',
        },
        { status: 500 }
      );
    }
    ...
    if (!TWILIO_APP_SID) {
      return NextResponse.json(
        {
          error:
            'Outbound browser calls require TWILIO_APP_SID (TwiML App SID). Configure it in .env.local to prevent call disconnects.',
        },
        { status: 500 }
      );
    }
